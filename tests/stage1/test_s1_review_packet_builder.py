...
def _fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for module, filename in EVIDENCE_ARTIFACTS:
        ...
        if filename == "work_identity_state_registry.json":
            atomic_json(root / filename, {"records": [{"work_id": "w1", "identity_state": "CURRENT"}]})
        else:
            _write(root / filename)
    atomic_json(stage / "m13" / "primary_review_registry.json", {"records": [{
    ...
    atomic_json(stage / "m14" / "independent_review_registry.json", {"records": [{
...
def test_independent_packet_is_blind_and_snapshot_bound(tmp_path: Path) -> None:
    ...
    assert packet["blindness_contract"]["primary_disposition_visible"] is False
...
def test_packet_tampering_is_detected(tmp_path: Path) -> None:
    ...
    template_path = tmp_path / "submission.json"
    payload = build_submission_template(out / "packet_index.json", template_path)
    payload["records"][0].update({"disposition": "ACCEPT", "evidence_ids": ["c1"], "rationale": "reviewed"})
    atomic_json(template_path, payload)
